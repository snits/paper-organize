...
class TestCLIIntegration:
    ...
    def teardown_method(self):
        ...
        if self.temp_path.exists():
            ...
            for directory in sorted(self.temp_path.rglob("*"), key=lambda p: len(str(p)), reverse=True):
                if directory.is_dir():
                    directory.rmdir()
            # Finally remove the temp directory itself
            self.temp_path.rmdir()

    def test_successful_download_with_real_http_endpoint(self):
    ...
    def test_automatic_pdf_extension(self):
    ...
    def test_connection_error_handling(self):
        ...
        assert result.exit_code != 0
        assert "✗ Network error:" in result.output
        assert "Connection failed" in result.output or "Name or service not known" in result.output
